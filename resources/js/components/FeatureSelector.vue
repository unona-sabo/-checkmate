...
const allSelected = computed(() => {
    return (
        localFeatures.value.length > 0 &&
        localFeatures.value.every((f) => props.modelValue.includes(f.id))
    );
});
...
const toggleAll = () => {
    if (allSelected.value) {
        ...
        emit(
            'update:modelValue',
            localFeatures.value.map((f) => f.id),
        );
    }
};
...
const createFeature = async () => {
    ...
    try {
        const response = await axios.post(
            `/projects/${props.projectId}/features`,
            {
                name: newFeature.value.name,
                module: newFeature.value.module
                    ? [newFeature.value.module]
                    : null,
...
<template>
    <div class="space-y-2">
                ...
                <DialogContent class="max-w-sm">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="space-y-1">
                            ...
                            <Input
                                id="new-feature-module"
                                v-model="newFeature.module"
        ...
        <div v-if="localFeatures.length" class="rounded-lg border">
            ...
            <label
                class="flex cursor-pointer items-center gap-2 border-t px-3 py-1.5 hover:bg-muted/50"
            >
                <Checkbox
            ...
            <div class="max-h-48 overflow-y-auto border-t px-2 py-1">
                <template v-if="filteredFeatures.length">
                    <label
                        ...
                        :key="feature.id"
                        class="flex cursor-pointer items-center gap-2 rounded px-1 py-1.5 hover:bg-muted/50"
                    >
                        <Checkbox
