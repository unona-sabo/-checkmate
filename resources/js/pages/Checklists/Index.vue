...
const filteredChecklists = computed(() => {
    ...
    if (filterFeature.value === '__none__') {
        result = result.filter(
            (checklist) =>
                !checklist.project_features ||
                checklist.project_features.length === 0,
        );
    } else if (filterFeature.value) {
        result = result.filter((checklist) =>
            checklist.project_features?.some(
...
const hasCategories = computed(
    () =>
        ...
        localChecklists.value.some((c) => c.category),
);
...
const onDropOnCard = (
    ...
    if (
        !draggedChecklist.value ||
        draggedChecklist.value.id === targetChecklist.id
    ) {
        onDragEnd();
        return;
    }
...
const isDraggingCategory = computed(
    () => draggedCategoryName.value !== undefined,
);
...
const onCategoryDragStart = (e: DragEvent, categoryName: string | null) => {
    ...
    e.dataTransfer!.setData(
        'text/plain',
        `category:${categoryName ?? '__null__'}`,
    );
};
...
const onDropCategoryOnCategory = (targetCategoryName: string | null) => {
    ...
    const draggedItems = list.filter(
        (c) => (c.category || null) === draggedCat,
    );
    ...
    const targetIdx = remaining.findIndex(
        (c) => (c.category || null) === targetCategoryName,
    );
    ...
    try {
        ...
        if (saved) {
            ...
            if (parsed && !Array.isArray(parsed) && parsed.content) {
                ...
                drafts.value = parsed.filter(
                    (d: NoteDraft) => d.content && d.content.trim(),
                );
            }
        }
    } catch (e) {
                ...
                >
                    ...
                    <div v-if="availableFeatures.length > 0" class="relative">
                        <Select v-model="filterFeature">
                            <SelectTrigger
                                class="h-8 w-44 bg-gradient-to-b from-background to-muted/30"
            ...
            <div v-else class="space-y-6">
                ...
                >
                    ...
                    <div
                        ...
                        "
                        class="group/header flex items-center gap-2 rounded-md px-2 py-1.5 transition-colors select-none hover:bg-muted/50"
                        :class="{
                            'bg-primary/5 ring-1 ring-primary/20':
                                isDragging && dragOverCategory === group.name,
                            'opacity-50':
                                isDraggingCategory &&
                                draggedCategoryName === group.name,
                            'cursor-pointer': true,
                        }"
                        @click="
                            renamingCategory !== group.name &&
                        ...
                        "
                        @dragstart.stop="
                            onCategoryDragStart($event, group.name)
                        "
                        @dragend="onDragEnd"
                    >
                        ...
                        <div
                            ...
                            class="shrink-0 cursor-grab opacity-0 transition-opacity group-hover/header:opacity-100 active:cursor-grabbing"
                            @click.stop
                        >
                            <GripVertical
                        ...
                        >
                            <input
                                ...
                                type="text"
                                class="w-48 rounded border border-input bg-background px-2 py-0.5 text-sm font-medium outline-none focus:ring-1 focus:ring-ring"
                                @click.stop
                        ...
                        <RestrictedAction>
                            <button
                                v-if="
                                    ...
                                    renamingCategory !== group.name
                                "
                                class="cursor-pointer rounded p-0.5 opacity-0 transition-opacity group-hover/header:opacity-100 hover:bg-muted"
                    ...
                    >
                        <div
                            ...
                            @dragover.prevent.stop="
                                onDragOver($event, group.name)
                            "
                            @drop.stop="
                                onDropOnCard($event, checklist, group.name)
                            "
                        >
                            <!-- Drag handle -->
                            <div
                                v-if="canDrag"
                                class="absolute top-2 right-2 z-10 cursor-grab rounded p-1 opacity-0 transition-opacity group-hover/card:opacity-100 hover:bg-muted active:cursor-grabbing"
                                @mousedown.stop
                            >
                                <GripVertical
                                    class="h-4 w-4 text-muted-foreground"
                            ...
                            >
                                <Card
                                    class="flex h-full cursor-pointer flex-col transition-all hover:border-primary"
                                    :class="{
                                        'opacity-50':
                                            isDragging &&
                                            draggedChecklist?.id ===
                                ...
                                >
                                    <CardHeader class="flex-1">
                                        <CardTitle
                                            class="flex items-start gap-2 pr-8"
                                        >
                                            <ClipboardList
                                            ...
                                            <RestrictedAction>
                                                <DropdownMenu v-if="canDrag">
                                                    ...
                                                    >
                                                        <button
                                                            @click.prevent.stop
                                                            class="ml-auto inline-flex cursor-pointer items-center gap-1 rounded-full px-2 py-0.5 text-xs transition-colors"
                                                            :class="
                                                                checklist.category
                                                                    ? 'border border-amber-500/20 bg-amber-500/10 text-amber-700 hover:bg-amber-500/20 dark:text-amber-400'
                                                                    : 'bg-muted text-muted-foreground opacity-0 group-hover/card:opacity-100 hover:bg-muted/80'
                                                            "
                                                        >
                                                            <FolderOpen
                                                                class="h-3 w-3"
                ...
                <div
                    v-if="
                        ...
                        (searchQuery.trim() || filterFeature)
                    "
                ...
                <DialogContent class="max-w-sm">
                    ...
                    <div class="py-4">
                        <Input
                            ...
                            @keydown.enter="
                                editingChecklistId &&
                                setNewCategory(editingChecklistId)
                            "
                        />
                    </div>
