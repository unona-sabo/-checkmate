...
const activeFilterCount = computed(
    () => allFilters.filter((f) => f.value !== '').length,
);
...
const filteredTestRuns = computed(() => {
    ...
    return props.testRuns.filter((run) => {
        ...
        // Source filter
        if (filterSource.value && run.source !== filterSource.value)
            return false;

        // Priority filter
        if (filterPriority.value && run.priority !== filterPriority.value)
            return false;

        // Environment filter
        if (
            filterEnvironment.value &&
            run.environment !== filterEnvironment.value
        )
            return false;
        ...
        if (
            filterAuthor.value &&
            String(run.creator?.id) !== filterAuthor.value
        )
            return false;

        // Created date filters
        const createdDate = run.created_at.slice(0, 10);
        if (filterCreatedFrom.value && createdDate < filterCreatedFrom.value)
            return false;
        if (filterCreatedTo.value && createdDate > filterCreatedTo.value)
            return false;

        // Completed date filters
        const completedDate = run.completed_at?.slice(0, 10) ?? '';
        if (filterCompletedFrom.value) {
            if (!completedDate || completedDate < filterCompletedFrom.value)
                return false;
        }
        if (filterCompletedTo.value) {
            if (!completedDate || completedDate > filterCompletedTo.value)
                return false;
        }

        // Passed range
        const passed = run.stats?.passed ?? 0;
        if (filterPassedMin.value && passed < Number(filterPassedMin.value))
            return false;
        if (filterPassedMax.value && passed > Number(filterPassedMax.value))
            return false;

        // Failed range
        const failed = run.stats?.failed ?? 0;
        if (filterFailedMin.value && failed < Number(filterFailedMin.value))
            return false;
        if (filterFailedMax.value && failed > Number(filterFailedMax.value))
            return false;

        // Total checks range
        const total = run.test_run_cases_count ?? 0;
        if (filterTotalMin.value && total < Number(filterTotalMin.value))
            return false;
        if (filterTotalMax.value && total > Number(filterTotalMax.value))
            return false;
...
const pauseRun = (run: TestRun) => {
    router.post(
        `/projects/${props.project.id}/test-runs/${run.id}/pause`,
        {},
        { preserveScroll: true },
    );
};
...
const resumeRun = (run: TestRun) => {
    router.post(
        `/projects/${props.project.id}/test-runs/${run.id}/resume`,
        {},
        { preserveScroll: true },
    );
};
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    >
                        <Play class="mt-1 h-6 w-6 shrink-0 text-primary" />
                        Test Runs
                    </h1>
                ...
                >
                    ...
                    <div class="grid grid-cols-3 gap-x-3 gap-y-2.5">
                        ...
                        <div class="relative">
                            ...
                            <Select v-model="filterSource">
                                <SelectTrigger
                                    class="h-8 cursor-pointer text-xs"
                                    :class="filterSource ? 'pr-7' : ''"
                                >
                                    <SelectValue placeholder="All" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="test-cases"
                                        >Test Cases</SelectItem
                                    >
                    ...
                    <div class="mt-2.5 grid grid-cols-3 gap-x-3 gap-y-2.5">
                        <div class="relative">
                            ...
                            <Select v-model="filterEnvironment">
                                <SelectTrigger
                                    class="h-8 cursor-pointer text-xs"
                                    :class="filterEnvironment ? 'pr-7' : ''"
                                >
                                    <SelectValue placeholder="All" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem
                                        v-for="env in uniqueEnvironments"
                                        :key="env"
                                        :value="env"
                                        >{{ env }}</SelectItem
                                    >
                            ...
                            <button
                                v-if="filterEnvironment"
                                @click="filterEnvironment = ''"
                                class="absolute right-1.5 bottom-1.5 z-10 cursor-pointer rounded-full p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground"
                            >
                                <X class="h-3 w-3" />
                            </button>
                    ...
                    <div class="mt-2.5 grid grid-cols-3 gap-x-3 gap-y-2.5">
                        ...
                        <div class="relative">
                            <Label
                                class="mb-1 block text-[11px] text-muted-foreground"
                                >Completed From</Label
                            >
                            <Input
                                v-model="filterCompletedFrom"
                                type="date"
                                class="h-8 text-xs"
                                :class="filterCompletedFrom ? 'pr-7' : ''"
                            />
                            <button
                                v-if="filterCompletedFrom"
                                @click="filterCompletedFrom = ''"
                                class="absolute right-1.5 bottom-1.5 z-10 cursor-pointer rounded-full p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground"
                            >
                                <X class="h-3 w-3" />
                            </button>
                        ...
                        <div class="relative">
                            ...
                            >
                            <Input
                                v-model="filterCompletedTo"
                                type="date"
                                class="h-8 text-xs"
                                :class="filterCompletedTo ? 'pr-7' : ''"
                            />
                            <button
                                v-if="filterCompletedTo"
                                @click="filterCompletedTo = ''"
                                class="absolute right-1.5 bottom-1.5 z-10 cursor-pointer rounded-full p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground"
                            >
                                <X class="h-3 w-3" />
                            </button>
                    ...
                    <div class="mt-3 flex justify-end">
                        <span class="text-sm text-muted-foreground">
                            ...
                            {{ filteredTestRuns.length === 1 ? 'run' : 'runs' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div
                v-else-if="filteredTestRuns.length === 0"
            ...
            >
                ...
                >
                    No test runs match "{{ searchQuery }}"
                </p>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        >
                                            <span
                                                v-if="run.environment"
                                                v-html="
                                                    highlight(run.environment)
                                                "
                                            />
                                            <span
                                                v-if="run.milestone"
                                                v-html="
                                                    highlight(run.milestone)
                                                "
                                            />
                                            <span
                                                >{{
                                                    run.test_run_cases_count ||
                                                    0
                                                }}
                                                cases</span
                                            >
                                            ...
                                            >
                                                ...
                                                <span
                                                    v-if="run.completed_by_user"
                                                    class="flex items-center gap-0.5"
                                                >
                                                    <User class="h-3 w-3" />
                                                    {{
                                                        run.completed_by_user
                                                            .name
                                                    }}
                                                </span>
                                            </template>
                                ...
                                <div class="flex shrink-0 items-center gap-2">
                                    <RestrictedAction>
                                        <button
                                            ...
                                            class="cursor-pointer rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-yellow-500/10 hover:text-yellow-500"
                                            title="Pause"
                                        >
                                            <Pause class="h-4 w-4" />
                                        </button>
                                    ...
                                    <RestrictedAction>
                                        <button
                                            ...
                                            class="cursor-pointer rounded-md p-1.5 text-yellow-500 transition-colors hover:bg-green-500/10 hover:text-green-500"
                                            title="Resume"
                                        >
                                            <Play class="h-4 w-4" />
                                        </button>
