...
const preselectedParentId = urlParams.get('parent_id')
    ? Number(urlParams.get('parent_id'))
    : null;
const preselectedParent = preselectedParentId
    ? props.parentSuites.find((s) => s.id === preselectedParentId)
    : null;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: preselectedParent ? 'Create Subcategory' : 'Create',
        href: `/projects/${props.project.id}/test-suites/create`,
    },
];
...
<template>
    <Head
        :title="preselectedParent ? 'Create Subcategory' : 'Create Test Suite'"
    />
                    ...
                    <CardHeader>
                        ...
                        <CardDescription>
                            <template v-if="preselectedParent">
                                ...
                                <strong>{{ preselectedParent.name }}</strong>
                            </template>
                            <template v-else>
                                Create a new test suite to organize your test
                                cases.
                            </template>
                        </CardDescription>
                    </CardHeader>
                        ...
                        <form @submit.prevent="submit" class="space-y-4">
                            ...
                            <div class="flex gap-2">
                                ...
                                <Button
                                    ...
                                    @click="
                                        $inertia.visit(
                                            `/projects/${project.id}/test-suites`,
                                        )
                                    "
                                >
                                    Cancel
